...
import { getWalletByRoot, postRevokeCreds } from '@miw/APIs/VcManagement.api';
import {
  IssueDismantler,
  IssueFramework,
  IssueGenericCreds,
...
import { Button, CustomAccordian, CustomInput, Dialog, Pagination, ThreeDotItemMenu } from '@miw/stories';
...
const WalletAccordianHeader = ({
  ...
  const handleRevokeCreds = () => {
    ...
    postRevokeCreds(param)
      .then((res) => {
        getAlert('success', t('REVOCATION.REVOKED_MSG'));
        refreshList();
      })
      .catch((err) => {
  ...
  return (
    <div className={StyledVcMgmt.headerContainer}>
      ...
      <p className={StyledVcMgmt.type}>{formatDate(getUTCOfsetToZero(createdAt), 'yyyy-MM-dd | HH:mm:ss')}</p>
      <div className={StyledVcMgmt.buttonGroup} onClick={(e) => e.stopPropagation()}>
        ...
        <Button isLoading={isRevoking} onClick={handleRevokeCreds} disabled={!didDocument?.credentialStatus}>
          {t('VC_MANAGEMENT.REVOKE')}
        </Button>
      </div>
      <div className={StyledVcMgmt.dialogue} onClick={(e) => e.stopPropagation()}>
        <Dialog
        ...
        <Dialog
          ...
          content={<CreartePresentation didDocument={didDocument} onClose={() => setIsOpenPresentDialoge(false)} />}
...
export const WalleteDetails = ({ didJson }: { didJson: WalletProps }) => {
  ...
  return (
    <div className={StyledVcMgmt.bodyContainer}>
      <pre className={StyledVcMgmt.jsonContainer}>{JSON.stringify(didJson, null, 2)}</pre>
...
const VcManagemanegement = (props: Props) => {
  ...
  const [issueCertificateDialogue, setIssueCertificateDialogue] = useState<CertificateType>(null);
  const menuItems = [
  ...
  const handleMenuClick = (clickedItem: { id: number; value: CertificateType; label: string }) => {
    ...
    <section className={StyledVcMgmt.container}>
      <div className={StyledVcMgmt.header}>
        ...
        <div className={StyledVcMgmt.headerRight}>
          <ThreeDotItemMenu menuItems={menuItems} handleItemClick={handleMenuClick} />
        </div>
      </div>
                  ...
                  <CustomAccordian
                    ...
                    accordionHeader={
                      <WalletAccordianHeader
                        ...
                        didDocument={wallet}
                        refreshList={callGetWalletsByRoot}
                        type={wallet?.type[1]}
      ...
      <Dialog
        ...
        showFooter={false}
        header={menuItems.find((item) => item.value === issueCertificateDialogue)?.label}
        key={'Create Wallet'}
