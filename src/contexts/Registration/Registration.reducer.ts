...
export type RegisterationState = {
    ...
    selectedEducationLevel?: unknown;
    imageCounter: number;
};
...
type validationPayload = { isValidated: true } | { isValidated: false; invalidDetails: invalidDetails };
...
export const RegistrationReducer = (state: RegisterationState, action: Action): RegisterationState => {
    switch (action.type) {
        case 'FETCHING': {
        ...
        case 'FETCHED_SUCCESS': {
            ...
            } else if (remainingRegistration !== null && completedRegistration !== null) {
            ...
            if (
                ...
                for (let i = 0; i < action.payload.entities.length; i++) {
                    if (action.payload.entities[i].entityUUID === state.selectedEntity.entityUUID) {
            ...
            if (action.payload.lastSelectedEntityUUID && !remainingRegistration && action.payload.entities.length > 1) {
                for (let i = 0; i < action.payload.entities.length; i++) {
                    if (
                        action.payload.entities[i].entityUUID === action.payload.lastSelectedEntityUUID &&
        ...
        case 'VALIDATION_FETCHED': {
            return {
                ...
                invalidatedDetails: action.payload.isValidated === true ? undefined : action.payload.invalidDetails,
            };
        }
